...
let MCOMPositionA: mod.Vector;
let MCOMPositionB: mod.Vector;
...
let attackersJailPoint = mod.GetSpatialObject(2501);
let defendersJailPoint = mod.GetSpatialObject(2502);
...
// const MCOMPositionA: mod.Vector = mod.CreateVector(300, 70, 129.5);
// const MCOMPositionB: mod.Vector = mod.CreateVector(332, 65.8, 104);

// When assigning spawn wall positions, order it by rotation vectors: ZERO, ZERO, [0, +/-90, 0], [0, +/-90, 0]
// const LevelData: Dict = {
//     "limestone": {
//         "MCOM_A": mod.CreateVector(300, 70, 129.5),
//         "MCOM_B": mod.CreateVector(332, 65.8, 104),
//         "SpawnWalls": mod.RuntimeSpawn_Limestone.ConcreteFoundation_01_1536x384_B,
//         "attackersSpawnWallsPos": [mod.CreateVector(294, 64, 53),
//             mod.CreateVector(294, 64, 65),
//             mod.CreateVector(294, 64, 53),
//             mod.CreateVector(309, 64, 53)],
//         "defendersSpawnWallsPos": [mod.CreateVector(329, 73.4, 132.2),
//             mod.CreateVector(329, 73.4, 144),
//             mod.CreateVector(329, 73.4, 132.2),
//             mod.CreateVector(344, 73.4, 132.2)]
//     }
// }

let attackerSpawnWalls: mod.SpatialObject[] = [];
let defendersSpawnWalls: mod.SpatialObject[] = [];
let attackersSpawnWallsStartPos: mod.Vector[] = [];
let defendersSpawnWallsStartPos: mod.Vector[] = [];

// The map MUST have 4 spawn walls for attackers and defenders with the following Object IDs
let attackersSpawnWallIDs: number[] = [101, 102, 103, 104];
let defendersSpawnWallIDs: number[] = [201, 202, 203, 204];

let attackersJailPos: mod.Vector = mod.CreateVector(303, 152, 63);
let defendersJailPos: mod.Vector = mod.CreateVector(308, 152, 16);
...
export async function OnGameModeStarted() {
    ...
    // Always have objects representing the plant sites, with IDs 20 (A) and 21 (B)
    MCOMPositionA = mod.GetObjectPosition(mod.GetSpatialObject(20));
    console.log("world icon A at position: ", mod.XComponentOf(MCOMPositionA), ", ", mod.YComponentOf(MCOMPositionA), ", ", mod.ZComponentOf(MCOMPositionA));
    MCOMPositionB = mod.GetObjectPosition(mod.GetSpatialObject(21));
    console.log("world icon B at position: ", mod.XComponentOf(MCOMPositionB), ", ", mod.YComponentOf(MCOMPositionB), ", ", mod.ZComponentOf(MCOMPositionB));

    attackersJailPos = mod.GetObjectPosition(attackersJailPoint);
    defendersJailPos = mod.GetObjectPosition(defendersJailPoint);
    console.log("Attackers Jail is at position: ", mod.XComponentOf(attackersJailPos), ", ", mod.YComponentOf(attackersJailPos), ", ", mod.ZComponentOf(attackersJailPos));


    BombData.Initialize();

    for (let i = 0; i < attackersSpawnWallIDs.length; ++i) {
        let wall = mod.GetSpatialObject(attackersSpawnWallIDs[i]);
        attackerSpawnWalls.push(wall);
        attackersSpawnWallsStartPos.push(mod.GetObjectPosition(wall));
    }

    for (let i = 0; i < defendersSpawnWallIDs.length; ++i) {
        let wall = mod.GetSpatialObject(defendersSpawnWallIDs[i]);
        defendersSpawnWalls.push(wall);
        defendersSpawnWallsStartPos.push(mod.GetObjectPosition(wall));
    }
    ...
    mod.StopSound(alarmSFX_A);
    mod.StopSound(alarmSFX_B);
        ...
        if (BombData.carryingPlayer && mod.GetSoldierState(BombData.carryingPlayer, mod.SoldierStateBool.IsAlive)) {
            ...
            if (BombData.isPlanting) {
                if (BombData.carryingPlayer != null) {
                    ...
                    if (BombData.isAtA) {
                        let playerPos = mod.GetSoldierState(BombData.carryingPlayer, mod.SoldierStateVector.GetPosition);
                        let distA = mod.DistanceBetween(playerPos, MCOMPositionA);
                        if (distA > bombInteractDistance) {
                            console.log("Player position: ", mod.XComponentOf(playerPos), " ", mod.YComponentOf(playerPos), " ", mod.ZComponentOf(playerPos));
                            console.log("Too far from MCOM A position: ", mod.XComponentOf(MCOMPositionA), " ", mod.YComponentOf(MCOMPositionA), " ", mod.ZComponentOf(MCOMPositionA));
                        ...
                        let playerPos = mod.GetSoldierState(BombData.carryingPlayer, mod.SoldierStateVector.GetPosition);
                        let distB = mod.DistanceBetween(playerPos, MCOMPositionB);
                        if (distB > bombInteractDistance) {
                            console.log("Player position: ", mod.XComponentOf(playerPos), " ", mod.YComponentOf(playerPos), " ", mod.ZComponentOf(playerPos));
                            console.log("Too far from MCOM B position: ", mod.XComponentOf(MCOMPositionB), " ", mod.YComponentOf(MCOMPositionB), " ", mod.ZComponentOf(MCOMPositionB));
                            console.log("Distance is ", distB);
                            BombData.CancelPlant();
                        }
                    }
                }
                else {
                    console.log("Bomb carrying player is null while trying to plant!");
                    BombData.CancelPlant();
                }
...
export function OnPlayerDeployed(eventPlayer: mod.Player): void {
    ...
    if (jsPlayer) {
        ...
        if (buyPhase == true) {
            ...
            if (jsPlayer.outOfRound) {
                if (teamID == attackingTeamID) {
                    mod.Teleport(eventPlayer, attackersJailPos, 0);
                }
                else {
                    mod.Teleport(eventPlayer, defendersJailPos, 0);
                }
...
async function SetupRound() {
    ...
    for (let i = 0; i < attackerSpawnWalls.length; ++i) {
        let wall = attackerSpawnWalls[i];
        mod.MoveObjectOverTime(wall, mod.CreateVector(0, wallMoveDelta, 0), mod.CreateVector(0, 0, 0), 3, false, false);
    }

    for (let i = 0; i < defendersSpawnWalls.length; ++i) {
        let wall = defendersSpawnWalls[i];
        mod.MoveObjectOverTime(wall, mod.CreateVector(0, wallMoveDelta, 0), mod.CreateVector(0, 0, 0), 3, false, false);
    }
...
async function EndRound(winners: mod.Team): Promise<void> {
    ...
    for (let i = 0; i < attackerSpawnWalls.length; ++i) {
        let wall = attackerSpawnWalls[i];
        mod.MoveObject(wall, mod.Subtract(attackersSpawnWallsStartPos[i], mod.GetObjectPosition(wall)), ZEROVEC);
    }
    for (let i = 0; i < defendersSpawnWalls.length; ++i) {
        let wall = defendersSpawnWalls[i];
        mod.MoveObject(wall, mod.Subtract(defendersSpawnWallsStartPos[i], mod.GetObjectPosition(wall)), ZEROVEC);
    }
...
function GetMapName(): string {
    if (mod.IsCurrentMap(mod.Maps.Limestone)) {
        console.log("mod.IsCurrentMap() returned true on Limestone");
        return "limestone";
    }
    else if (mod.IsCurrentMap(mod.Maps.Abbasid))
        console.log("mod.IsCurrentMap() returned true on Abbasid");
    else if (mod.IsCurrentMap(mod.Maps.Aftermath))
        console.log("mod.IsCurrentMap() returned true on Aftermath");
    else if (mod.IsCurrentMap(mod.Maps.Badlands))
        console.log("mod.IsCurrentMap() returned true on Badlands");
    else if (mod.IsCurrentMap(mod.Maps.Battery))
        console.log("mod.IsCurrentMap() returned true on Battery");
    else if (mod.IsCurrentMap(mod.Maps.Capstone))
        console.log("mod.IsCurrentMap() returned true on Capstone");
    else if (mod.IsCurrentMap(mod.Maps.Dumbo))
        console.log("mod.IsCurrentMap() returned true on Dumbo");
    else if (mod.IsCurrentMap(mod.Maps.Eastwood))
        console.log("mod.IsCurrentMap() returned true on Eastwood");
    else if (mod.IsCurrentMap(mod.Maps.Firestorm))
        console.log("mod.IsCurrentMap() returned true on Firestorm");
    else if (mod.IsCurrentMap(mod.Maps.Outskirts))
        console.log("mod.IsCurrentMap() returned true on Outskirts");
    else if (mod.IsCurrentMap(mod.Maps.Tungsten))
        console.log("mod.IsCurrentMap() returned true on Tungsten");

    console.log("Not on map set up for Bomb Squad");
    return "limestone";
}
...
class BombData {
    ...
    static ItemDropped(player?: mod.Player) {
        ...
        this.bombModel = mod.SpawnObject(mod.RuntimeSpawn_Limestone.Pizzabox_01_Open, mod.Add(this.lastCarryingPlayerPos, mod.CreateVector(0, 0.25, 0)), ZEROVEC);
