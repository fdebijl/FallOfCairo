...
class TrackData {
    ...
    async DistanceCheckToCheckpoint() {
        ...
        while (this.trackState == TrackState.running || this.trackState == TrackState.winnerFound) {
		    ...
		    if (playerProfile.checkpoint == 0) {
		        mod.PlayMusic(mod.MusicEvents.BR_InsertionLanding, playerProfile.player);
		    }
		    mod.PlaySound(SFX_Checkpoint, 2.8, playerProfile.player);
...
let SFX_Checkpoint: mod.SFX;
let SFX_ReadyUp: mod.SFX;
let SFX_UnreadyUp: mod.SFX;
let SFX_MenuNavigation: mod.SFX;
let SFX_MenuHover: mod.SFX;
...
export async function OnGameModeStarted() {
    ...
    const SFX_Checkpoint_Id = mod.SpawnObject(
	    mod.RuntimeSpawn_Common.SFX_UI_Gauntlet_Beacons_CalibrationTick_OneShot2D,
	    mod.CreateVector(0, 0, 0),
	    mod.CreateVector(0, 0, 0));
    SFX_Checkpoint = mod.GetSFX(SFX_Checkpoint_Id)

    const SFX_ReadyUp_Id = mod.SpawnObject(
	    mod.RuntimeSpawn_Common.SFX_UI_MainMenu_PressPlay_OneShot2D,
	    mod.CreateVector(0, 0, 0),
	    mod.CreateVector(0, 0, 0));
    SFX_ReadyUp = mod.GetSFX(SFX_ReadyUp_Id)

    const SFX_UnreadyUp_Id = mod.SpawnObject(
	    mod.RuntimeSpawn_Common.SFX_UI_MenuNavigation_Challenges_Untrack_OneShot2D,
	    mod.CreateVector(0, 0, 0),
	    mod.CreateVector(0, 0, 0));
    SFX_UnreadyUp = mod.GetSFX(SFX_UnreadyUp_Id);

    const SFX_MenuNavigation_Id = mod.SpawnObject(
	    mod.RuntimeSpawn_Common.SFX_UI_MenuNavigation_Home_SelectPlayCategory_OneShot2D,
	    mod.CreateVector(0, 0, 0),
	    mod.CreateVector(0, 0, 0));
    SFX_MenuNavigation = mod.GetSFX(SFX_MenuNavigation_Id);

    const SFX_MenuHover_Id = mod.SpawnObject(
	    mod.RuntimeSpawn_Common.SFX_UI_MenuNavigation_Default_HoverIn_OneShot2D,
	    mod.CreateVector(0, 0, 0),
	    mod.CreateVector(0, 0, 0));
    SFX_MenuHover = mod.GetSFX(SFX_MenuHover_Id);
}
...
class HoH_UIVehicleSelect {
    ...
    buttonFocused(widgetName: string, focusIn: boolean = false) {
        ...
        mod.PlaySound(SFX_MenuHover, 1, this.#playerProfile.player);
    }
    ...
    buttonPressed(widgetName: string) {
	    ...
	    mod.PlaySound(SFX_MenuNavigation, 0.5, this.#playerProfile.player);
            ...
            if (!this.#playerProfile.readyUp) {
		        ...
		        mod.PlaySound(SFX_ReadyUp, 1, this.#playerProfile.player);

            } else {
                this.#playerProfile.readyUp = false;
		        mod.PlaySound(SFX_UnreadyUp, 1, this.#playerProfile.player);
            }
